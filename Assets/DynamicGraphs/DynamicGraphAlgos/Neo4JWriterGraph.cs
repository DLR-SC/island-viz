...
public class Neo4JWriterGraph
{
    ...
    public static IEnumerator WritePositionsToDB(List<Dictionary<string, object>> parameterList)
    {
        string statement = "UNWIND {paramList} as row " +
            "MATCH(n) WHERE id(n) = row.id " +
            "SET n.posX = row.posX " +
            "SET n.posZ = row.posZ";
        Dictionary<string, object> dict = new Dictionary<string, object> { { "paramList", parameterList } };
    ...
    public static IEnumerator SetCommitsLayouted(List<Commit> commits)
    {
        List<int> commitIds = new List<int>();
        foreach(Commit c in commits)
        {
            commitIds.Add(c.GetNeoId());
        }
        string statement = "MATCH (n) WHERE id(n) in $paramList " +
                     "SET n.graphLayouted = true";
